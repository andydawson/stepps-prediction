...
bw <- readShapeLines('data/map_data/bw/bw_albers.shp',
                     proj4string=CRS('+init=epsg:3175'))
...
plot_data_maps_binned_bw <- function(r, centers, taxa, N, K, breaks, limits, suff, save_plots, fpath=subDir){
  ...
  p <- theme_clean(p) + 
    ...
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())#+ theme(strip.text = element_text(size = 12),
                                    #       axis.text = element_text(size=14),
                                    #       axis.title = element_text(size=14))
