	...
	for (int t=0; t<T; ++t) {
	  for (int i=0; i<N_cores; ++i) {
		...
		for (int c=0; c<N; ++c) {
		  ...
		  const double drnew1 = (idx_cores[i]-1 == c) ? gamma[k] : (1-gamma[k]) * (w[k](i,c) * sum_w[k][si] - w[k](i,c) * out_sum_si_m) * res * res * invsumw2;
		  const double drnew2 = (idx_cores[i]-1 == c) ? 0.0 : (1-gamma[k]) * (-w[k](i,c) * out_sum_si_m) * res * res * invsumw2;
