...
public:
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        pos__ = 0U;
        context__.validate_dims("initialization", "alpha_t", "vector_d", context__.to_vec((K * T),N_knots));
        std::vector<vector_d> alpha_t((K * T),vector_d(N_knots));
        for (int j1__ = 0U; j1__ < N_knots; ++j1__)
            for (int i0__ = 0U; i0__ < (K * T); ++i0__)
   ...
   void transform_inits(const stan::io::var_context& context,
    ...
    double log_prob_grad(vector<double>& params_r,
      ...
      paleon::Timer timer_lpg(1), timer_fac(1), timer_varg(K), timer_rnew(1), timer_lgamma(1),
	timer_mvn(K), timer_gnormal(K), timer_alphat(K), timer_dirmult(K);
      ...
      //std::cout << "omega init : " << omega << std::endl;
      
      //vector<double> lambda_inv(K);
      vector<double> omega2(K);

      // vector<matrix_d> Q_s(K);
      // vector<matrix_d> q_s(K);
      // vector<matrix_d> Q_s_L(K);
      // vector<matrix_d> Q_s_inv(K);
      ...
      // vector<matrix_d> H_s(K);
      vector<matrix_d> H_t(K);
	...
	lp_thrd[k] += multi_normal_cholesky_log_double(alpha_t[k*T], zeros, C_s_L[k], false);

	for (int t=1; t<T; ++t){
	  lp_thrd[k] += multi_normal_cholesky_log_double(alpha_t[k*T + t], omega[k] * alpha_t[k*T + t-1], sqrt((1 - omega2[k])) * C_s_L[k]);
      	}

	H_t[k] = c_s[k] * C_s_inv[k];

	vector<vector_d> qQinv_alpha(T);
	for (int t=0; t<T; ++t){
	  qQinv_alpha[t] = H_t[k] * alpha_t[k*T + t];
	}
	...
	timer_varg.tic(k);
      	...
      	for (int i = 0; i<N; ++i){
      	  ...
      	  mu_g[k][i * T] = mu[k] + mu_g[k][i * T]  + qQinv_alpha[0][i];;
      	  var_g[k][i * T] = eta2[k] - cvar + 1 - qvar[k][i];;

      	  for (int t=1; t<T; ++t){
      	    mu_g[k][i * T + t] = mu[k] + mu_t[k][t-1] + mu_g[k][i * T + t] +  qQinv_alpha[t][i];
      	    var_g[k][i * T + t] = eta2[k] - cvar +  (1-omega2[k]) -  (1-omega2[k]) * qvar[k][i];
      	  }
      	}
	...
	alpha_Qinv_alpha_first = (alpha_t[k*T]).transpose() * C_s_inv[k] * alpha_t[k*T];
	...
	for (int t=1; t<T; ++t){
	  ...
	  alpha_Qinv_alpha_full = (alpha_t[k*T+t] - omega[k] * alpha_t[k*T+t-1]).transpose() * C_s_inv[k] * (alpha_t[k*T+t] - omega[k] * alpha_t[k*T+t-1]);

	  double alpha_Qinv_alpha_d1;
	  alpha_Qinv_alpha_d1 =  ( (alpha_t[k*T+t] - omega[k] * alpha_t[k*T+t-1]).transpose() * C_s_inv[k] *  alpha_t[k*T+t-1]);

	  gradient[1 + k] +=  N_knots * omega[k] / ( 1 - omega2[k]);
	  gradient[1 + k] +=  - omega[k] / (1 - omega2[k]) / (1 - omega2[k]) * alpha_Qinv_alpha_full;
	  gradient[1 + k] += 1 / (1 - omega2[k]) *  alpha_Qinv_alpha_d1;
	}
	...
	for (int t=0; t<T; ++t){
	  for (int n=0; n<N; ++n){
	    ...
	    int idx_g        = 1 + K*2 + K*(T-1) + K*N_knots + K*T*N_knots + k*N*T + n*T + t;
	    ...
	    if (t > 0){
	      // wrt sigma
	      gradient[1 + k] += (1 / B - AoverB * AoverB ) * omega[k] * (1 - qvar[k][n]);
	    } 
	    ...
	    //if (t > 0){
	      for (int v=0; v<N_knots; ++v){
	      	int idx_alpha_t = 1 + K*2 + K*(T-1) + K*N_knots + (k*T + t)*N_knots + v;
	...
	// gradient[1 + K + k] = gradient[1 + K + k] * lambda_ja[k] + lambda_dj[k];
	...
	for (int t=0; t<T; ++t) {
	  ...
	  if (t>0) {
	    Qinv_alphat_now = - 1 / (1 - omega2[k]) *  C_s_inv[k] * (alpha_t[k*T+t] - omega[k] * alpha_t[k*T+t-1]);
	  } else {
	    Qinv_alphat_now = - C_s_inv[k] * alpha_t[k*T+t];
	  }

	  if (t < (T-1)) {
	    Qinv_alphat_next =  omega[k] / (1-omega2[k]) * C_s_inv[k] * (alpha_t[k*T+t+1] - omega[k] * alpha_t[k*T+t]);
	  }
	...
	for (int t=0; t<T; ++t) {
	  for (int i=0; i<N_cores; ++i) {
		...
		for (int c=0; c<N; ++c) {
		  ...
		  const double drnew = (idx_core == c) ? gamma[m] : w[m](i,c) * drnew_case2;
		  ...
		  // g
		  int idx = 1 + K*2 + K*(T-1) + K*N_knots + K*T*N_knots + k*N*T + c*T + t;
