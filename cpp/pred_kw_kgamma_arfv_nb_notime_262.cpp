...
private:
    ...
    double const mu_std = 2; //from 5 // used to be 20!
...
public:
	...
	//w = res * res * w;
        ...
        //++num_params_r__; //ksi
        //num_params_r__ += K; //omega
        num_params_r__ += K; //mu
	//num_params_r__ += (T-1) * K; //mu_t
        num_params_r__ += N_knots * K; //alpha_s
        //num_params_r__ += N_knots * (K * (T-1)); //alpha_t
        num_params_r__ += (N * T) * K; //g
    }
   ...
   void transform_inits(const stan::io::var_context& context,
	...
	timer_varg.tic(k);
	  ...
	  qvar[k][i] = cvar;
      	  ...
      	  for (int t=1; t<T; ++t){
      	    mu_g[k][i * T + t] = mu[k] + mu_g[k][i * T + t];
      	    var_g[k][i * T + t] = eta2[k] - cvar;
      	  }
      	}
	...
	for (int t=0; t<T; ++t){
	  for (int n=0; n<N; ++n){
	    ...
	    int idx_g        = K + K*N_knots + k*N*T + n*T + t;
	    ...
	    //if (t > 0){
	    //  // wrt sigma
	    //  gradient[k] += (1 / B - AoverB * AoverB ) * (eta2[k] - qvar[k][n]);
	    //} 
	...
	for (int t=0; t<T; ++t) {
	  for (int i=0; i<N_cores; ++i) {
		...
		for (int c=0; c<N; ++c) {
		  ...
		  // g
		  int idx = K + K*N_knots + k*N*T + c*T + t;
