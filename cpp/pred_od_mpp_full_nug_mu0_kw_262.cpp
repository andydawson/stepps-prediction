   ...
   void transform_inits(const stan::io::var_context& context,
	...
	for (int i = 0; i < N_cores; ++i) {
	  for (int t = 0; t < T; ++t) {
	    ...
	    r_new(i * T + t, k) = gamma * r(idx_core, k);
	    for (int j = 0; j < N; ++j) {
	    ...
	    r_new(i * T + t, k) += out_sum(i *T + t, k) * (1 - gamma) / sum_w[k][i*T+t];
	  }
	}
      }
	...
	for (int t=0; t<T; ++t) {
	  for (int i=0; i<N_cores; ++i) {
		...
		const double drnew_case2 = (1-gamma) * res * res / sum_w_pot[m]; 
		...
		for (int c=0; c<N; ++c) {
		  ...
		  const double drnew = (idx_core == c) ? gamma : w[m](i,c) * drnew_case2;
