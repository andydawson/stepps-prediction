...
namespace pred_model_namespace {
  ...
  typedef Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic> matrix_d;
    ...
    static double const ksi_mean = 0.1;
    static double const ksi_std = 0.005; // used to be 20!
...
public:
	...
	for (int k = 0; k < K; ++k) {
	  gamma[k] = vals_r__[pos__++];
        ...
        stan::math::validate_non_negative_index("omega_alpha", "K", K);
        omega_alpha = vector_d(K);
        context__.validate_dims("data initialization", "omega_alpha", "vector_d", context__.to_vec(K));
        vals_r__ = context__.vals_r("omega_alpha");
        pos__ = 0;
        size_t omega_alpha_i_vec_lim__ = K;
        for (size_t i_vec__ = 0; i_vec__ < omega_alpha_i_vec_lim__; ++i_vec__) {
            omega_alpha[i_vec__] = vals_r__[pos__++];
        }
        omega_beta = vector_d(K);
        context__.validate_dims("data initialization", "omega_beta", "vector_d", context__.to_vec(K));
        vals_r__ = context__.vals_r("omega_beta");
        pos__ = 0;
        size_t omega_beta_i_vec_lim__ = K;
        for (size_t i_vec__ = 0; i_vec__ < omega_beta_i_vec_lim__; ++i_vec__) {
            omega_beta[i_vec__] = vals_r__[pos__++];
        }
	...
	//   check_greater_or_equal(function__,"res",res,0);
	  ...
	  check_greater_or_equal(function__,"phi",phi,0);
        ...
        try {
	  check_greater_or_equal(function__,"omega_alpha",omega_alpha,0);
        } catch (std::domain_error& e) {
            throw std::domain_error(std::string("Invalid value of omega_alpha: ") + std::string(e.what()));
        };
        try {
	  check_greater_or_equal(function__,"omega_beta",omega_beta,0);
        } catch (std::domain_error& e) {
            throw std::domain_error(std::string("Invalid value of omega_beta: ") + std::string(e.what()));
        };
        // try {
	...
	ones = vector_d(N);
        ...
        // stan::math::validate_non_negative_index("M", "(N * T)", (N * T));
        // M = matrix_d((N * T),(N * T));
	stan::math::validate_non_negative_index("M_H_s", "K", K);
   ...
   //  }
    ...
    double normal_log_double(double y, const double mu, const double sigma) const {
      double lp = 0.0;
      double inv_sigma = 1.0/sigma;
    ...
    double beta_log_double(const double y, const double alpha, const double beta) 
const {
      double lp = 0.0;
      double log_1my = log(1-y);
      double log_y = log(y);

      const double alpha_beta = alpha + beta;

      lp += (alpha-1) * log_y + (beta-1)*log_1my-lgamma(alpha)-lgamma(beta)+lgamma(alpha_beta);
    ...
    double log_prob_grad(vector<double>& params_r,
      ...
      lp += normal_log_double(ksi, ksi_mean, ksi_std);

      for (int k=0; k<K; ++k) {
        lp += beta_log_double(omega[k], omega_alpha[k], omega_beta[k]);
      }
	...
	for (int t=1; t<T-1; ++t)
      ...
      gradient[0] +=  (ksi_mean - ksi) / (ksi_std * ksi_std);
	...
	// omega jacobian adjustments
	gradient[1 + k] += (omega_alpha[k]-1)/omega[k] + (omega_beta[k]-1)/(omega[k]-1);
