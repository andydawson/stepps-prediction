...
private:
    ...
    static double const sigma_ub = 1;
...
public:
   ...
   void transform_inits(const stan::io::var_context& context,
	...
	// mu_t[k][0]
	gradient[1 + 2*W + k*(T-1)] -= inv_ksi2 * mu_t[k][0];
	gradient[1 + 2*W + k*(T-1)] += inv_ksi2 * (mu_t[k][1] - mu_t[k][0]);

	// partial of g wrt mu_t
      	for (int n=0; n<N; ++n){
      	  for (int t=0; t<T-1; ++t){
      	    double idx = n*T + t+1;
      	    gradient[1 + 2*W + k*(T-1) + t] += (g[k](idx) - mu_g[k][idx]) / var_g[k][idx];
      	  }
      	}
	...
	for (int v=0; v<N_knots; ++v) {
	  int idx_alpha_s = 1 + W*2 + W*(T-1) + k*N_knots + v;
	...
	// partial of mu prior
	gradient[1 + W + k] -=  mu[k] / (mu_std * mu_std);
	...
	for (int t=0; t<T; ++t){
	  ...
	  for (int n=0; n<N; ++n){
	    ...
	    gradient[1 + W + k] += AoverB;

	    int idx_g        = 1 + W*2 + W*(T-1) + W*N_knots + W*(T-1)*N_knots + k*N*T + n*T + t;
	    ...
	    if (t > 0){
	      for (int v=0; v<N_knots; ++v){
		int idx_alpha_t = 1 + W*2 + W*(T-1) + W*N_knots + (k*(T-1) + t-1)*N_knots + v;
	...
	for (int t=0; t<(T-1); ++t) {
	  ...
	  for (int v=0; v<N_knots; ++v){
	    int idx_alpha_t = 1 + W*2 + W*(T-1) + W*N_knots + (k*(T-1) + t)*N_knots + v;
	...
	for (int t=0; t<T; ++t) {
	  for (int i=0; i<N_cores; ++i) {
		...
		for (int c=0; c<N; ++c) {
		  ...
		  // g
		  int idx = 1 + W*2 + W*(T-1) + W*N_knots + W*(T-1)*N_knots + k*N*T + c*T + t;
